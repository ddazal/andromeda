...
class Andromeda:
    ...
    def _call_ollama_api(
        ...
        try:
            ...
            st.error(
                f"Error communicating with Ollama API: {e.status_code} - {e.response}"
            )
            return None
    ...
    def generate_go_tests(
        self, function_code: str, status_reporter: Any = None
    ) -> dict:
        """
        ...
        try:
            ...
            if status_reporter:
                status_reporter.update(
                    label="Analyzing Go function...", state="running", expanded=True
                )
            ...
            """
            analysis = self._call_ollama_api(
                system_prompt_analysis, prompt_analysis, 0.3
            )
            if analysis is None:
            ...
            if status_reporter:
                status_reporter.update(
                    label="Generating test cases...", state="running", expanded=True
                )
                status_reporter.write(
                    "Based on analysis, formulating diverse test scenarios..."
                )
            ...
            """
            tests = self._call_ollama_api(
                system_prompt_test_cases, prompt_test_cases, 0.4
            )
            if tests is None:
            ...
            if status_reporter:
                status_reporter.update(
                    label="Generating Go test code...", state="running", expanded=True
                )
                status_reporter.write(
                    "Translating test cases into executable Go test code..."
                )
            ...
            """
            go_test_code = self._call_ollama_api(
                system_prompt_go_tests, prompt_go_tests, 0.2
            )
